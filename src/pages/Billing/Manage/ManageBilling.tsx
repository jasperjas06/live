      ...
      if (response?.data?.data) {
        ...
        reset({
          ...
          paymentDate: empData.paymentDate
            ? new Date(empData.paymentDate).toISOString().split("T")[0]
            : "", // Format date
          status:
            empData.status && empData.status.toLowerCase().includes("enquir")
              ? "enquired"
              : empData.status || "enquired",
        ...
        if (id && !append) {
          ...
          if (formValues.customerId && newdata.length > 0) {
            const foundCustomer = newdata.find(
              (option: any) => option.value === formValues.customerId,
            );
            if (foundCustomer) {
              setSelectedCustomer(foundCustomer);
            }
        ...
        if (res.status === 200) {
          ...
          if (billingMonth === "previous" || billingMonth === "current") {
            ...
            if (currentMonthEmi) {
              ...
              // Set Amount
              setValue("amount", currentMonthEmi.emiAmt, {
                shouldValidate: true,
              });
              ...
              <FormSection>
                ...
                <Grid container spacing={3}>
                  <Grid size={{ xs: 12, sm: 12, md: 6 }}>
                    <Autocomplete
                      ...
                      renderOption={(props, option) => {
                        ...
                        const displayIdentifier = option.id
                          ? option.id
                          : option._id;
                        const searchTerm = debouncedCustomerSearch
                          .toLowerCase()
                          .trim();
                  ...
                  {(modeOfPayment.toLowerCase() === "card" ||
                    modeOfPayment.toLowerCase() === "online") && (
                    <Grid size={{ xs: 12, sm: 12, md: 6 }}>
                      <Controller
                        control={control}
                        name="referenceId"
